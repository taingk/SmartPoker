...
function		send_raise_limits(table, game, seat_nb)
{
	var			curseat;
	var			raise_limit1;
	...
	if (curseat.state == "playing")
	{
		...
		console.log('raise limit1 : '+raise_limit1); /** 2;(game.curbet - curseat.bet) * 2 > 0 ? (game.curbet - curseat.bet) * 1.10 : game.curbet;
...
function		send_blinds(table, sb, bb)
{
	...
	io.to(table.id).emit("bet", table.game.sb_pos, pb == null ? sb.bet = 10 : sb.bet = pb/*sb.bet = cfg.conf.small_blind*/);
	io.to(table.id).emit("bet", table.game.bb_pos, bb.bet = cfg.conf.big_blind);

	console.log('sb : ' + sb.bet, 'bb : ' + bb.bet);
}

function		ask_first_player(socket, player, game)
{
