...
router.get("/pokertable-:name", function(request, response)
{
	...
	{
		response.status(404);
		response.sendFile("public/table_not_found.html", {root: __dirname });
	}
});
...
router.get("/seat-:name", function(request, response)
{
	if (tables_ids.indexOf(request.params.name.substr(5)) != -1)
	{
		...
		{
			response.status(404);
			response.sendFile("public/2nd_screen/page_not_found.html", {root: __dirname });
		}
		response.sendFile("public/2nd_screen/login.html", {root: __dirname });
	}
	else if (request.params.name == "disconnected.html")
	{
		response.sendFile("public/2nd_screen/disconnected.html", {root: __dirname });
	}
	else
	{
		response.status(404);
...
io.on("connection", function(socket)
{
    ...
    if ((table = get_table(table_id, tables)) == "not found" && !device_client)
    {
    	...
    	curgame = new Game(deck, -1, -1, doublesb/*cfg.conf.small_blind*/, -1, doublebb/*cfg.conf.big_blind*/, 0, 0, 0, 0, "waiting", 0, new Array());
